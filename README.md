...
- **Let them debate.** Add two running bots to a channel's permissions and they can @mention each other there without copying a channel ID. They **stop after 4 turns** (configurable), and `/stop-all` ends it at once.
...
| **Apps → Ask bot name** | right-click or long-press a message | Send that message and its attachments to the chosen bot |

Slash command responses are visible only to the requester. Choosing a conversation from another channel in `/resume` detaches it there. Normal requests still use @mentions or DMs.
...
- **Debates are bounded:** at most `MAX_BOT_TURNS`, then a human must step in; `/stop-all` can end them early.
