...
- **토론시키기:** 지정한 채널에서는 봇끼리 서로 멘션하며 주고받습니다. **4턴 후 멈추고**(바꿀 수 있음), `!stop`으로 바로 끝낼 수 있습니다.
...
- macOS 또는 Linux, Node.js 18 이상 (`ctl.sh`는 zsh가 필요한데, macOS에는 기본으로 있습니다)
...
Discord에서 **사용자 설정 → 고급 → 개발자 모드**를 켭니다. 그다음 채널을 우클릭해 **채널 ID 복사하기**, 내 이름을 우클릭해 **사용자 ID 복사하기**를 합니다.
...
| `DEBATE_CHANNEL_IDS` | 없음 | 봇끼리 서로 부를 수 있는 채널 |
...
- **접근:** 봇은 `ALLOWED_CHANNEL_IDS` 채널(과 DM)에서만 답하고, `ALLOWED_USER_IDS`를 설정하면 그 사람들에게만 답합니다.
...
| 문제 | 해결 |
|---|---|
| 채널에서 봇이 답을 안 함 | 채널 ID를 `ALLOWED_CHANNEL_IDS`에, 내 ID를 `ALLOWED_USER_IDS`에 넣고 `./ctl.sh restart <이름>`을 실행합니다. |
...
| `#debate`에서 봇끼리 태그를 안 함 | 두 봇 모두 한 번 이상 실행돼서 `state/agents.json`에 등록돼 있어야 하고, 그 채널이 두 봇의 `DEBATE_CHANNEL_IDS`에 모두 있어야 합니다. |
