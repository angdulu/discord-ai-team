...
- **작업 진행 상태 표시:** 봇은 대화 사이에 연결을 유지하고 타이핑 표시를 갱신합니다. 한 봇에게 직접 요청하면 영어로 된 짧은 작업 상태와 경과 시간을 별도 메시지에 보여주면서 기존처럼 답변 초안도 갱신합니다. 완료되면 진행 메시지는 사라지고 최종 답변이 남습니다. 토론 채널에서는 봇 간 호출이 이어지도록 완성된 답변을 별도로 올립니다. Codex와 Claude는 도구 작업 종류를, Gemini는 일반적인 작업 진행 상태를 보여줍니다. 여러 봇에게 한 요청이나 봇끼리 이어가는 턴은 완성된 답변만 올립니다.
