...
- **모델 바로 바꾸기:** `/model`을 치면 모델과 추론 강도를 고르는 드롭다운이 뜹니다. 선택값은 현재 채널에 적용되고 새 채널의 기본값이 됩니다. 기존 채널의 선택값은 유지됩니다.
...
- **작업 진행 상태 표시:** 봇은 대화 사이에 연결을 유지하고 타이핑 표시를 갱신합니다. 한 봇에게 직접 요청하면 별도 메시지에 영어로 된 짧은 상태(`Thinking …`, 이후 경과 시간과 확인 가능한 도구 작업)를 보여줍니다. 토론이 꺼진 채널에서는 기존처럼 답변 초안도 갱신합니다. 토론 채널에서는 진행 메시지를 지우고 완성된 답변을 새로 올려 봇 간 호출을 이어갑니다. 여러 봇에게 한 요청이나 봇끼리 이어가는 턴은 완성된 답변만 올립니다.
...
`/model`에서 현재 채널과 앞으로 만드는 새 채널의 모델과 추론 강도를 고를 수 있습니다.
...
**일단 써보고 싶다면:** 함께 들어 있는 예시를 이 폴더 밖으로 복사하고(`cp -r example-workspace ~/leftover-demo`), `WORKSPACE_DIR`를 그 복사본으로 지정하세요. 토큰이 이 저장소 안에 있으니, 봇이 저장소 안의 폴더를 보게 하면 안 됩니다. 가상의 사이드 프로젝트(냉장고 속 재료로 저녁을 추천하는 앱 "Leftover")의 노트와 공유 규칙이 들어 있어서, 봇들에게 미정 질문을 토론시켜 볼 수 있습니다. 봇의 작업 폴더를 다른 곳으로 설정했다면 `example-workspace`는 실행에 쓰이지 않습니다.
