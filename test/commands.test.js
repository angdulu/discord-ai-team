...
test('every management action is a slash command and each bot has a message menu', () => {
  const commands = commandDefinitions('Claude Bot');
  ...
  assert.deepEqual(commands.filter((command) => command.type === ApplicationCommandType.Message).map((command) => command.name), ['Ask Claude Bot']);
...
test('team commands can be left to another bot', () => {
  const names = commandDefinitions('Claude Bot', { teamCommands: false }).map((command) => command.name);
